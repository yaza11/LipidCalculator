...
def find_inductive_cleavage_positions(mol: Chem.Mol) -> list[tuple[int, int]]:
    ...
    for atom in mol.GetAtoms():
        ...
        for atom_nghbr in atom.GetNeighbors():
            if atom_nghbr.GetSymbol() not in SUPPORTED_OTHER_ATOMS:
                continue
            if atom_nghbr.GetFormalCharge() != 0:
                continue
            if mol.GetBondBetweenAtoms(atom_nghbr.GetIdx(), atom.GetIdx()).GetBondType() != BondType.SINGLE:
                continue
...
if __name__ == "__main__":
    ...
    # mol = AddHs(Chem.MolFromSmiles('CC[O+]CC'))
    mol = AddHs(Chem.MolFromSmiles('[OH3+]'))
    # draw with indices
